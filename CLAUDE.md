...
When the user reports game results or a round finishes, read `agents/results-analyst.md` and follow its protocol. This tracks prediction accuracy, documents misses with the delta between pick and result, and generates confidence calibration and per-agent accuracy stats.

Each round's results write to `output/<round-name>/`:
- `results.json` — predicted vs actual for every reported game
- `retrospective.md` — miss breakdowns, hit summaries, and round-level stats

Run this incrementally (game-by-game as results come in) or as a batch when a full round wraps.
...
- `results.json` — predicted vs actual results (generated by results analyst)
- `retrospective.md` — miss analysis, confidence calibration, agent accuracy (generated by results analyst)
